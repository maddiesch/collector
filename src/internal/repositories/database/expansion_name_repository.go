...
func (d *Database) ExpansionNameSearchPrefix(ctx context.Context, prefix string, name string) ([]string, error) {
	...
	stmt := statement.Select("SetName").Distinct().From("Cache_DefaultCard").Where(where).OrderBy("SetName", true)
...
func (d *Database) ExpansionNameForCard(ctx context.Context, name string) ([]string, error) {
	stmt := statement.Select("SetName").Distinct().From("Cache_DefaultCard").Where(
		conditional.Equal("Name", name),
	).OrderBy("SetName", true)
